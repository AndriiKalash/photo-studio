...
const forms = () => {
  ...
  const path = {
    ...
    // для формы с классом popup-design
  ...
  form.forEach(element => {
    element.addEventListener('submit', e => {
      ...
      statusMasage.classList.add("status"); // div с картинкой загрузки помещаем в родитель самой формы

      element.parentNode.appendChild(statusMasage); //скрываю форму(чтоб осталоaсь тоько каринка оповещения)
      ...
      }, 4000); //добавляю картинку загрузки 

      let statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMasage.appendChild(statusImg); // добавляю текст к картинке
